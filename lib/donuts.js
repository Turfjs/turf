  ...
  _.each(fc.features, function(poly1){
    _.each(fc.features, function(poly2){
      ...
      if(_.isEqual(poly1, poly2)){
        ...
        contained(poly1, poly2, function(isContained){
          // if it is contained and has different properties, erase poly2 from poly1 and push the result to donuts
          if(isContained && !_.isEqual(poly1.properties, poly2.properties)){
            // erase poly2 from poly1 and push
            if(poly1.properties.density === 10 && poly2.properties.density === 117){
              console.log('problem')
            }
            t.erase(poly1, poly2, function(err, erased){
          ...
          // if it is contained and has the same properties, merge poly1 and poly2 and push the result to donuts
          if(isContained && _.isEqual(poly1.properties, poly2.properties)){
            // merge poly1 and poly2 and push
            t.union(poly1, poly2, function(err, unioned){
