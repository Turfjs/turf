...
const intersects = require('./dist/js/index.js').default;
...
test('turf-boolean-intersects', t => {
    ...
    glob.sync(path.join(__dirname, 'test', 'true', '**', '*.geojson')).forEach(filepath => {
        ...
        if (process.env.SHAPELY) shapely.intersects(feature1, feature2).then(result => t.true(result, '[true] shapely - ' + name));
    ...
    glob.sync(path.join(__dirname, 'test', 'false', '**', '*.geojson')).forEach(filepath => {
        ...
        if (process.env.SHAPELY) shapely.intersects(feature1, feature2).then(result => t.false(result, '[false] shapely - ' + name));
