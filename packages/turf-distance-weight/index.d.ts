 ...
 * @param {number} [options.threshold] If the distance between neighbor and target features is greater than threshold,
 *  the weight of that neighbor is 0.
 * @param {number} [options.p] Minkowski p-norm distance parameter.
 * 1: Manhattan distance. 2: Euclidean distance. 1=<p<=infinity.
 * @param {boolean} [options.binary] If true, weight=1 if d <= threshold otherwise weight=0.
 *  If false, weight=Math.pow(d, alpha).
 * @param {number} [options.alpha] distance decay parameter.
