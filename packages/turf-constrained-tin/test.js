...
test('tin - z property', t => {
    ...
    const newResult = newTin(mercator, [[2, 12]]);
    fs.writeFileSync(path.join(__dirname, 'test', 'newTinMercator.json'), JSON.stringify(newResult, null, 2));
...
test('tin - grid without edges', t => {
    ...
    const tinned = newTin(points_grid, []);
    t.equal(tinned.features[0].geometry.type, 'Polygon');
...
test('tin - grid with 1 edge', t => {
    ...
    const tinned = newTin(points_grid, [[0, 5]]);
    t.equal(tinned.features[0].geometry.type, 'Polygon');
    t.equal(tinned.features.length, 8);
    t.deepEqual(tinned, expected, 'tinned polygons match');
...
test('tin - grid with 2 edges', t => {
    ...
    const tinned = newTin(points_grid, [[0, 5],[5, 6]]);
    t.equal(tinned.features[0].geometry.type, 'Polygon');
    t.equal(tinned.features.length, 8);
    t.deepEqual(tinned, expected, 'tinned polygons match');
