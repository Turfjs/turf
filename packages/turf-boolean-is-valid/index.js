"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var invariant_1 = require("@turf/invariant");
var helpers_1 = require("@turf/helpers");
var boolean_disjoint_1 = require("@turf/boolean-disjoint");
var boolean_crosses_1 = require("@turf/boolean-crosses");
var line_intersect_1 = require("@turf/line-intersect");
var boolean_point_on_line_1 = require("@turf/boolean-point-on-line");
/**
 * booleanIsValid checks if the geometry is a valid according to the OGC Simple Feature Specification.
 *
 * @name booleanIsValid
 * @param {Geometry|Feature<any>} feature GeoJSON Feature or Geometry
 * @returns {boolean} true/false
 * @example
 * var line = turf.lineString([[1, 1], [1, 2], [1, 3], [1, 4]]);
 *
 * turf.booleanIsValid(line, point);
 * //=true
 */
function booleanIsValid(feature) {
    var geom = invariant_1.getGeom(feature);
    var type = invariant_1.getType(feature);
    var coords = invariant_1.getCoords(feature);
    switch (type) {
        case 'Point':
            return coords.length > 1;
        case 'MultiPoint':
            for (var i = 0; i < coords.length; i++) {
                if (coords[i].length < 2)
                    return false;
            }
            return true;
        case 'LineString':
            if (coords.length < 2)
                return false;
            for (var i = 0; i < coords.length; i++) {
                if (coords[i].length < 2)
                    return false;
            }
            return true;
        case 'MultiLineString':
            if (coords.length < 2)
                return false;
            for (var i = 0; i < coords.length; i++) {
                if (coords[i].length < 2)
                    return false;
            }
            return true;
        case 'Polygon':
            for (var i = 0; i < geom.coordinates.length; i++) {
                if (geom.coordinates[i].length < 4)
                    return false;
                if (!checkRingsClose(geom.coordinates[i]))
                    return false;
                if (checkRingsForSpikesPunctures(geom.coordinates[i]))
                    return false;
                if (i > 0) {
                    if (line_intersect_1.default(helpers_1.polygon([geom.coordinates[0]]), helpers_1.polygon([geom.coordinates[i]])).features.length > 1)
                        return false;
                }
            }
            return true;
        case 'MultiPolygon':
            for (var i = 0; i < geom.coordinates.length; i++) {
                var poly = geom.coordinates[i];
                for (var ii = 0; ii < poly.length; ii++) {
                    if (poly[ii].length < 4)
                        return false;
                    if (!checkRingsClose(poly[ii]))
                        return false;
                    if (checkRingsForSpikesPunctures(poly[ii]))
                        return false;
                    if (ii === 0) {
                        if (!checkPolygonAgainstOthers(poly, geom.coordinates, i))
                            return false;
                    }
                    if (ii > 0) {
                        if (line_intersect_1.default(helpers_1.polygon([poly[0]]), helpers_1.polygon([poly[ii]])).features.length > 1)
                            return false;
                    }
                }
            }
            return true;
    }
}
exports.default = booleanIsValid;
function checkRingsClose(geom) {
    return geom[0][0] === geom[geom.length - 1][0] || geom[0][1] === geom[geom.length - 1][1];
}
function checkRingsForSpikesPunctures(geom) {
    for (var i = 0; i < geom.length - 1; i++) {
        var point = geom[i];
        for (var ii = i + 1; ii < geom.length - 2; ii++) {
            var seg = [geom[ii], geom[ii + 1]];
            if (boolean_point_on_line_1.default(point, helpers_1.lineString(seg)))
                return true;
        }
    }
    return false;
}
function checkPolygonAgainstOthers(poly, geom, index) {
    var polyToCheck = helpers_1.polygon(poly);
    for (var i = index + 1; i < geom.length; i++) {
        if (!boolean_disjoint_1.default(polyToCheck, helpers_1.polygon(geom[i]))) {
            if (boolean_crosses_1.default(polyToCheck, helpers_1.lineString(geom[i][0])))
                return false;
        }
    }
    return true;
}
